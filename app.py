...
def inject_notifications():
    if 'user_id' in session:
        ...
        payments = db.execute('''
            ...
            WHERE user_id = ? AND date(julianday(due_date)) - date(julianday(strftime('%Y-%m-%d', 'now'))) <= 2
        ''', (session['user_id'],)).fetchall()

        # Fetch upcoming meetings
        meetings = db.execute('''
            SELECT title, 'meeting' as type FROM meetings
            WHERE user_id = ? AND date(julianday(date)) - date(julianday(strftime('%Y-%m-%d', 'now'))) <= 1
        ''', (session['user_id'],)).fetchall()
...
def meeting():
    db = get_db()
    meetings = db.execute('SELECT * FROM meetings WHERE user_id = ? ORDER BY date, time', (session['user_id'],)).fetchall()
...
def pagamenti():
    db = get_db()
    payments_raw = db.execute('SELECT * FROM payments WHERE user_id = ?', (session['user_id'],)).fetchall()
